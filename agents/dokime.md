...
dokime-checkpoint <run_id> <ticket_id> <step> <outcome>
                  ...
                  [--review <concept> <deck> <result> <difficulty> [<target_difficulty>]]   (repeatable, at Step 7)
...
**Legitimate skip reasons (T9 convention).** The Step-3 comprehension check is universal but gentle — and it scales to zero on dev-authored content. When the concept being checked IS the dev's own framing (the ticket request came from the dev's own work, not from a third-party spec or a stakeholder request), the check becomes ceremony. Skip with `--comprehension skipped:dev_authored` and log the reason. The `bureaucratic_nudge` discipline applies to the comprehension layer itself, not just to question content. Tagged class: `comprehension_check_on_dev_authored`.

**At Step 7 (Propose Approach):** review relevant prior knowledge *before* proposing the approach. Surface cards that pertain to this ticket via `bin/list-relevant-cards <project> <files>` (narrow) and `bin/list-skill-cards` (broad-candidate pool); combine narrow + dev-curated broad; sort by `leitner_box` ascending; cap at **3**; pose one discriminating question per surviving card; add one `--review <concept> <deck> <result> <difficulty> [<target_difficulty>]` to this checkpoint's `dokime-checkpoint` call per reviewed card. Reviews update the card AND emit a `Step 7:review` comprehension record; they do **not** emit `checkpoint-outcomes` (the cross-check stays clean). Empty deck or no relevant cards → skip silently. Full directive lives in Step 7 below.

**Adaptive difficulty (T8 — calibration foundations).** Before each comprehension question (Step 3) or review (Step 7), call `bin/target-bloom <concept>` to compute the target Bloom level (cold-start → `recall`); aim the AI's discriminating question at that level. Pass the target into `dokime-checkpoint` so the record carries it: for **Step-3 comprehension**, via the top-level `--target-difficulty <bloom>` flag (one check per call, one target); for **Step-7 reviews**, via the optional 5th positional arg of `--review` itself (each card may need a different target). When a target is recorded the comprehension-check record bumps to `schema_version: 2` and includes `target_difficulty`; without it, records stay at `schema_version: 1`. v1 *instruments* (the target-vs-self-tag gap is auditable in the records); v2+ will *tune*.

**Four-pass delivery ritual (T9 — Bloom-laddered staircase).** Step 3 / Step 6 / Step 7 / Step 11 form a four-pass delivery sequence — **Silent → What → How → Why** — that matches `bin/target-bloom`'s ladder. Each pass attaches to the workflow step that already does that level of work: Silent at Step 3 (image first, UI walkthrough); What at Step 6 (3-words-per-step recall of governing conventions); How at Step 7 (sentence-form proposal using "by"); Why at Step 11 (justification using "because"; common-mistake content lands here). When recording the per-step comprehension question, add `--comprehension-pass <pass>` so the record carries which delivery pass produced it; that bumps the comprehension-check record to `schema_version: 3`. Pass selection — which subsequent pass plays at full volume rather than compressed — happens at Step 7 using the MIN `leitner_box` across surfaced cards; full directive lives in Step 7 below. Optionally, when a card is captured (`--card`) you may classify it via `--card-label <label>` (`essential_detail` / `most_common_mistake` / `safety_tip`); that bumps the card's `schema_version: 2` and lets the deck have labelled spikes-and-valleys for downstream consumers. The ritual is read-by-spirit — on COLLAPSE-scale tickets it compresses to single sentences; on FULL-LOOP it earns its place. Anti-COLLAPSE is the load-bearing constraint, not the ritual itself.
...
- Write to the spec file:
  ...
  - Dokime version — `vX.Y.Z` (e.g. `v1.19.0`); records the workflow shape that produced this spec, so a post-mortem reader can distinguish "the workflow lacked that step at the time" from "the author skipped it." Source: `/dokime:version` or the plugin manifest. Tagged class: `missing_version_traceability`.
...
**Paired-agent symmetry check (T9 / T10 lesson — recurring).** Whenever a ticket's framing claims work should be "symmetric to," "mirroring," or "the parallel of" a change in the sibling agent (`dokime.md` ↔ `dokime-review.md`), pause at Step 4 and verify the symmetry against the sibling's actual current state — and against any cited principles that intentionally differentiate the two. The review skill is by Bacchelli 2013 (cited inline at `dokime-review.md` line 326) intentionally lighter on style/comprehension than the dev workflow; forcing structural symmetry collapses Bacchelli's "second pair of eyes" value into "testing machine." Recurred twice in 2026-05-23 (T10 caught at Step 6, T9 caught at Step 10) — promoted from observation to Step-4 question. Tagged class: `false_symmetry_between_paired_agents`.

**Credit-then-discriminate (T9; triggers on ≥2 credible candidates).** When an ambiguity has multiple credible candidate defaults, do not jump to the recommended one. Instead, for each candidate: (a) state it; (b) name the operating condition under which the *wrong* candidate would actually work (give it credit — name a real case where someone might reasonably choose it); (c) put the alternatives to bed with discriminating evidence for the recommended choice. Single-credible-candidate ambiguities skip the credit step — just propose. The form teaches the dev the *reasoning*, not just the answer; it also surfaces the discriminating evidence the workflow needs to defend the recommendation against later challenge. Open-mindedness about the wrong answer is how the right answer earns its weight.
...
**Pending-decisions check (BL2 lesson).** For each file or area in the blast radius, grep recent ticket specs and the parent project's Decisions Log for *resolved-but-deferred* decisions about that file/area. If a pending decision exists, surface it explicitly before continuing: *"Decision D-X said this file moves; current ticket writes against current location. Confirm: keep writing here, or implement the deferred move first?"* Each subsequent ticket that writes against the wrong location reinforces the deferral and compounds the eventual cost (BL2: D12's directive deferred through T7/T8/T9, each accreting to the wrong location). Different from `deliberate_state_misclassified_as_bug` (which catches "this state is intentional — *don't change it*"); this catches "this state has a pending decision to change it — *consider implementing the decision instead of writing more around it*." Tagged class: `deferred_decision_compounded_against`.

**Downstream consumer audit (ICOV3-1538 lesson — QA escape).** For each table, API endpoint, service method, or shared data structure on the write path, grep for *consumers* — every read site, every caller. List them: file path + brief role (UI picker, integration sync, report query, cache warmer, etc.). For each consumer, capture the expected-before behavior — what shape/values it reads today, what state it assumes. Carry this list forward to Step 13 — every listed consumer earns a verification AC there. Spec-acknowledged risks (Step 5: "nobody on the team currently knows how the standard flow works") DO NOT substitute for the audit — the audit is the answer to the risk, not its restatement. Distinct from the framework-upgrade behavioral grep (which covers method-signature changes between framework major versions) — this generalizes to any change touching a shared data structure or contract: the migration produced data that no UI surface could read, because no step required identifying the picker as a downstream consumer. Tagged class: `downstream_consumer_blindspot`.

Document the relevant files, patterns, reusable components, and standards found.

**Pass 2 — What (T9 four-pass ritual; recall-level / Bloom 1).** The "Read the project first" + style sources + "Assess blast radius" work above *is* the What pass: summarise the governing conventions and the change surface in **3-words-per-step bullets** — *"guard clauses; pure functions; no `else`-after-`return`."* The dev confirms recognition. Pose one discriminating question at the `recall` Bloom level (or higher per the dev's Leitner state); record via `dokime-checkpoint --comprehension <result> <difficulty> --comprehension-pass what` (T9 — bumps record to `schema_version: 3` with `pass: "what"`). The discrimination test from Step 3 still applies — name a plausible wrong answer or regenerate.
...
4. **Conduct reviews:** for each surviving card, first call `bin/target-bloom <concept>` to compute the per-card target Bloom — aim the discriminating question at that level. Same discrimination rule as Step 3 (name a plausible wrong answer or regenerate). Pose; collect `pass` / `fail` + the question's Bloom difficulty.
5. **Record:** add one `--review <concept> <deck> <result> <difficulty> <target>` per reviewed card to this checkpoint's `dokime-checkpoint` call — the 5th arg is the per-card target you just computed (T8; bumps each review's comprehension record to `schema_version: 2` with `target_difficulty`). Also add `--comprehension-pass how` for the checkpoint's own comprehension question (T9 — marks Step 7 as Pass 3 of the four-pass ritual). See *Recording (single advance call)* at the top.
6. **Empty deck or no relevant cards → skip silently.** Non-gating; never blocks Step 7.
7. **MIN-box pass selection (T9).** With the relevant cards now known, compute `MIN(leitner_box)` across the surfaced narrow + curated-broad set. That value selects how Step 11's Why pass plays:
    - **MIN box 1–2 (cold)** → Step 11 Why at **default depth**; the dev still needs prior passes to land; running Why at full volume on top of weak earlier passes overloads.
    - **MIN box 3 (warm)** → Step 11 Why at **default depth**; balanced.
    - **MIN box 4–5 (hot)** → Step 11 Why at **FULL volume**; conventions cold, justification depth earns its place.
    - **No relevant cards (cold-start)** → all four passes at **compressed depth** (~30 s each); the cold-start ritual (Q-T9-3).

    Record the selection in the spec file's Decisions Log as a one-line note (`step-11-why-volume: full | default | compressed`). Component C derives the next-step volume from this and the recorded reviews; no separate machinery.
...
**Pass 4 — Why (T9 four-pass ritual; analyse / evaluate-level / Bloom 4–5).** Code-quality review *is* the Why pass: justification statements using the connector "because" — *"this guard returns early **because** the downstream resolver assumes a hydrated model; missing it would silently null-coalesce."* Common-mistake content lands here (the `most_common_mistake` card label). Run depth per Step 7's MIN-box selection: **FULL volume** if Step 7 designated `step-11-why-volume: full` (justify per change, name the alternative path that would have been wrong); **default depth** otherwise (justify the load-bearing decisions only). When recording the Step-11 comprehension check, add `--comprehension-pass why` (T9; the record carries `pass: "why"` at schema v3). If a card capture surfaces an MCM-shaped concept, classify it with `--card-label most_common_mistake` so the deck has labelled spikes-and-valleys for downstream consumers.
...
**Exercise downstream consumers (ICOV3-1538 lesson — QA escape).** Every consumer listed in Step 6's downstream consumer audit earns a falsifiable AC here. For each: drive the consumer with the data shape the implementation produces, verify behavior is either *unchanged* (the consumer is invariant to the change) or *explicitly intended-changed* (the spec named this consumer as in-scope and the new behavior matches the spec). Silent-empty, silent-broken, or "the migration command can manage this but no UI can" outcomes count as FAIL — the consumer was not exercised before ship. Spec match + persona-based AC pass is necessary but not sufficient when downstream consumers were not exercised; ICOV3-1538 passed Step 13 with three personas on the gate and still produced 184 subprograms of unmanageable data because the picker — a downstream consumer — was never driven against the new shape. Tagged class: `downstream_consumer_blindspot`.
